...
import { SearchAgencyByNameService } from '../services/agency-service';
...
export default function Agency() {
    ...
    const onSearchAgencyDataHandler = async (nama_instansi: string) => {
        const { response, data, refreshToken } = await SearchAgencyByNameService({ nama_instansi });
        if (response.ok) {
            if (refreshToken) {
    ...
    return (
        <Layout title='SI-KOP-BANGKIT | Instansi'>
                        ...
                        {agencyData &&
                            (agencyData.length === 0 ? (
                                ...
                                agencyData.map((agency, index) => (
                                    <tr key={agency.kode_ins} className='hover:bg-slate-600 transition-all duration-100'>
            ...
            {openAddModal && <AddAgencyModal setOpenAddModal={setOpenAddModal} lastId={agencyData[agencyData.length - 1].kode_ins} />}
        </Layout>
    );
}
