...
export default function Invoice() {
    ...
    const total = listedData.length > 0 ? listedData.map((dt) => dt.jumlah).reduce((total, curr) => total + curr) : 0;
    ...
    const getAgencyData = async () => {
        const { response, refreshToken, data } = await GetAgencyDataService();
        if (response.ok) {
            if (refreshToken) {
    ...
    const checkInvoiceData = async () => {
        const invoiceData = JSON.parse(localStorage.getItem('SERIALIZED_DATA') as string) || [];
    ...
    useEffect(() => {
        getAgencyData();
        setListedData(JSON.parse(localStorage.getItem('LISTED_DATA') as string) || []);
        setNotListedData(JSON.parse(localStorage.getItem('NOT_LISTED_DATA') as string) || []);
    }, []);
    ...
    return (
        <Layout title='SI-KOP-BANGKIT | Tagihan'>
            <div className='flex flex-col gap-4 md:flex-row justify-between'>
                <h2 className='text-3xl font-semibold text-slate-200'>Halaman Tagihan</h2>
                {listedData.length > 0 || notListedData.length > 0 ? (
                    <div className='flex gap-4'>
                        <RefreshButton doRefresh={doRefresh} loading={loading} />
                        <button className='px-4 py-2 bg-teal-600 text-slate-200 rounded-md hover:bg-teal-500 transition-all duration-300' onClick={onDownloadXLSX}>
            ...
            <div>
                <h2 className='text-slate-500 text-xl'>Total : {formatNumber(total)}</h2>
            </div>
