...
import { AddNewMutationDataService } from '../../services/mutation-service';
...
export default function AddMutationModal({ setOpenModal, agencyName }: AddMutationModalProps) {
    ...
    const addInputHandle = () => {
        setMutationData([...mutationData, { nama: '', dari: '', ke: '', bulan: thisMonth }]);
    };
    ...
    const onSubmitMutationHandle = async () => {
        ...
        try {
            parsedData = mutationData.map((data) => {
                const kdInsDari = agencyName.filter((agency) => agency.nama_ins === data.dari)[0].kode_ins;
                const kdInsKe = agencyName.filter((agency) => agency.nama_ins === data.ke)[0].kode_ins;
        ...
        const { response, data } = await AddNewMutationDataService({ mutationData: parsedData });
        if (response.ok) {
            setOpenModal(false);
        } else {
    ...
    return (
        <div>
            ...
            <div className='w-[95%] max-h-[85%] overflow-auto bg-slate-800 rounded-lg p-4 md:p-8 fixed z-50 shadow-xl top-10 left-1/2 -translate-x-1/2 transition-all backdrop-brightness-75 sm:w-[90%] lg:w-5/6 xl:w-4/6'>
                <div className='flex flex-col'>
                                    ...
                                    <tr key={i}>
                                        ...
                                        <td className='p-1'>
                                            <input
                                                ...
                                                onChange={(ev) => onChangeNameHandle(ev.target.value, i)}
                                            />
                                        </td>
                                        <td className='p-1'>
                                            <input
                                                ...
                                                value={dt.dari}
                                                className='w-full px-4 py-2 bg-slate-600 text-slate-200 rounded-md outline-none focus:ring-2 focus:ring-slate-500 transition-all'
                                                onChange={(ev) => onChangeFromHandle(ev.target.value, i)}
                                            />
                                        </td>
                                        <td className='p-1'>
                                            <input
                                        ...
                                        <td className='p-1'>
                                            <input
                                                ...
                                                onChange={(ev) => onChangeMonthHandle(ev.target.value, i)}
                                            />
                                        </td>
                                        <td className='p-1 text-center'>
                                            <button onClick={() => {}}>
                                                <DeleteButton onClick={() => onDeleteInputHandle(i)} className='text-red-500 hover:text-red-400' />
                                            </button>
                                        </td>
                                    </tr>
                                ))}
                    ...
                    <div className='flex gap-4 justify-end mt-2'>
                        <button className='p-1 border-2 border-emerald-600 rounded-md hover:bg-emerald-600 transition-all group' onClick={addInputHandle}>
                            <AddButton className='text-emerald-600 p-2 group-hover:text-white transition-all' />
                        </button>
                    </div>
                    <div className='flex w-full justify-end my-4'>
                        <button className='bg-emerald-600 px-4 py-2 text-white rounded-md hover:bg-emerald-500' onClick={onSubmitMutationHandle}>
                            Submit Data
                        </button>
                    </div>
                </div>
