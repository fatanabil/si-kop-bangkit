...
export default function NotListedDataModal({
  notListedData,
  setNotListedModalOpen,
  setAddMemberOpen,
  notListedModalOpen,
}: NotListedDataModalProps) {
  let total = 0;
  ...
  return (
    <div>
      ...
      <div className="w-5/6 h-5/6 absolute z-20 top-10 left-1/2 -translate-x-1/2 bg-slate-700 p-8 rounded-lg shadow-lg flex flex-col overflow-auto">
        ...
        <h4 className="text-2xl text-white my-4">
          {`Total = ${formatNumber(total)}`}
        </h4>
        <button
          className="px-4 py-2 bg-teal-600 rounded-md text-white hover:bg-teal-500"
          onClick={() => setNotListedModalOpen(false)}
        >
          Tutup
        </button>
        <button
