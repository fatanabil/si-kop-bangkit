...
export default function AddAgencyModal({
  ...
  const parsedID =
    parseInt(lastId.substring(lastId.indexOf("S") + 1, lastId.length)) + 1;
  const currID = () => {
  ...
  const handleOnAddInstansi = async () => {
    try {
      const post = await fetch(`${baseURL}/api/agency`, {
        method: "POST",
        body: JSON.stringify({ kode_ins: currID(), nama_ins: nmInstansi }),
        headers: {
        ...
        <div className="flex flex-col">
          ...
          <div className="flex flex-col mt-4">
            <label
              htmlFor="nama-ins"
              className={`text-xl ${
                response?.err ? "text-red-400" : "text-slate-200"
              } mb-2`}
            >
              Nama Instansi
            </label>
            <input
              ...
              className={`px-4 py-2 bg-slate-600 rounded-md outline-none focus:ring-2 transition-all ${
                response?.err
                  ...
                  : "text-slate-200 focus:ring-slate-500"
              }`}
          ...
          {response?.err && (
            <p className="text-red-400 mt-4">* {response.msg}</p>
          )}
