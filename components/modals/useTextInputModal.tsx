...
export default function UseTextInputModal({
  setOpenInputText,
  baseURL,
  setListedData,
  setNotListedData,
  setNotListedModalOpen,
}: UseTextInputModalProps) {
  ...
  return (
    <div>
      ...
      <div className="w-5/6 absolute z-20 top-10 left-1/2 -translate-x-1/2 bg-slate-700 p-8 rounded-lg shadow-lg flex flex-col overflow-auto">
        ...
        <h3 className="text-slate-400 mb-4">
          Format input : no_rek[tab]nama[tab]jumlah
        </h3>
        {/* {isFailed && (
          <Flash
            status={false}
        ...
        <textarea
          ...
          cols={30}
          rows={15}
          className="bg-slate-600 mb-4 text-white p-4"
          onChange={(e) => setDataText(e.target.value)}
        ></textarea>
        <button
          className="px-4 py-2 bg-teal-600 rounded-md text-white hover:bg-teal-500 flex justify-center items-center"
